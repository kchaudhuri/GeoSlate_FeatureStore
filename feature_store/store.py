...
class GeoFeatureStore:
    ...
    def _compute_geometry_hash(self, geometry: BaseGeometry) -> str:
        # Normalize and hash WKT
        wkt = geometry.wkt
        return hashlib.md5(wkt.encode()).hexdigest()

    def _check_duplicate(self, gdf: gpd.GeoDataFrame) -> bool:
    ...
    def save(self, gdf: gpd.GeoDataFrame, filename: str):
        ...
        logger.info(f"Saved GeoDataFrame to {output_path}")
    ...
    def load(self, filename: str) -> gpd.GeoDataFrame:
        ...
        logger.info(f"Loaded GeoDataFrame from {filepath} | {len(gdf)} records")
        return gdf
